...
export const UserPreview = ({ user, onToggleAdmin, onDeleteUser, openQuestionModal }) => {
    ...
    return <li className="user-preview">
        ...
        <section className='buttons'>
            ...
            <button className='sub-btn' onClick={() => onAdminClick(user._id, user.isAdmin, user.fullname)}>
                {user.isAdmin ? 'Remove admin' : 'Set admin'}
            </button>
            <button className='sub-btn' onClick={() => onDeleteClick(user._id, user.fullname)}>Delete</button>
        </section>
    </li>
}